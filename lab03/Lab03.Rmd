...
It looks pretty balanced. That is great, so we now gonna peform some models and avaliate its metrics.
...
knn_data <- data.frame(pred = knn_prediction, obs = train)
...
logistic_reg_data <- data.frame(pred = logistic_reg_prediction, obs = train$situacao)
...
```{r}
tree_prediction <- predict(model.tree_dec,train)

tree_data <- data.frame(pred = tree_prediction, obs = train$situacao)
